resource "aws_iam_role" "bbr-iam-lambda-notify-feedback" {
  name = "bbr-iam-lambda-notify-feedback"
...
resource "aws_iam_role_policy_attachment" "bbr-lambda-notify-feedback-logs" {
  role = "${aws_iam_role.bbr-iam-lambda-notify-feedback.name}"
...
resource "aws_lambda_function" "bbr-notify-feedback" {
  ...
  role = "${aws_iam_role.bbr-iam-lambda-notify-feedback.arn}"
...
resource "aws_iam_policy" "bbr-lambda-feedback-ses-policy" {
  name = "bbr-lambda-feedback-ses-policy"
  path = "/"
  description = "Allow access to SES in VPC"
...
resource "aws_iam_role_policy_attachment" "bbr-lambda-feedback-ses-access" {
  role = "${aws_iam_role.bbr-iam-lambda-notify-feedback.name}"
  policy_arn = "${aws_iam_policy.bbr-lambda-feedback-ses-policy.arn}"
}
