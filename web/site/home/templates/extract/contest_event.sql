INSERT INTO contest_event (old_id, updated, created, updated_by_id, owner_id, name, date_of_event, date_resolution, contest_id, notes, venue_id, complete, no_contest, contest_type_id, original_owner)
  VALUES (
    ...
    (SELECT id FROM user WHERE usercode='{{ContestEvent.contest.owner.username}}'),
    (SELECT id FROM user WHERE usercode='{{ContestEvent.contest.lastChangedBy.username}}'),
    '{{ContestEvent.name}}',
...
{% if ContestEvent.contest_type_override_link %}
  (SELECT if FROM contest WHERE old_id = {{ContestEvent.contest_type_override_link.id}}),
{% else %}
  (SELECT if FROM contest WHERE old_id = {{ContestEvent.contest.id}}),
{% endif %}
