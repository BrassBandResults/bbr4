...
def single_piece(request, pPieceSlug):
    ...
    if request.user.is_anonymous == False:
        ...
        if len(lUserOwnChoiceThisPiece):
            index = 1 #  without this it fails to sort for two results on the same date
            for lUserOwnChoiceHistory in lUserOwnChoiceThisPiece:
                lUserResultsForThisPieceWithDate.append((lUserOwnChoiceHistory.result.contest_event.date_of_event, index, lUserOwnChoiceHistory.result))
                index += 1
