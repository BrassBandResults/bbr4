resource "aws_iam_role" "bbrie-iam-lambda-thumbs" {
  name = "bbrie-iam-lambda-thumbs"
...
resource "aws_iam_role_policy_attachment" "bbr-lambda-thumbs-logs" {
  role = "${aws_iam_role.bbrie-iam-lambda-thumbs.name}"
...
resource "aws_lambda_function" "bbr-thumbnail" {
  ...
  role = "${aws_iam_role.bbrie-iam-lambda-thumbs.arn}"
...
resource "aws_iam_policy" "bbrie-lambda-access-thumbnails-policy" {
  name = "bbrie-lambda-access-thumbnails-policy"
  path = "/"
...
resource "aws_iam_role_policy_attachment" "bbr-lambda-write-to-thumbnails" {
  role = "${aws_iam_role.bbrie-iam-lambda-thumbs.name}"
  policy_arn = "${aws_iam_policy.bbrie-lambda-access-thumbnails-policy.arn}"
}
